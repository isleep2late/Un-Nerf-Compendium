...
public static class PokeList1
{
    ...
    {
        // PKHaX: emit the desynced "sprite" header species if one is set, else the data species.
        var species = pk.SpriteSpeciesInternal;
        if (species == 0)
            return SlotEmpty;
    ...
    public static PK1 ReadFromList(ReadOnlySpan<byte> input, int stringLength, int capacity = 1, bool isParty = true, int index = 0)
    {
        ...
        var pk = new PK1(body, stringSection1, stringSection2);
        // PKHaX: retain the list "sprite" species when it differs from the data-structure species.
        var header = input[1 + index];
        if (header is not (0 or SlotEmpty) && header != pk.SpeciesInternal)
            pk.HeaderSpeciesInternal = header;
        return pk;
    }
