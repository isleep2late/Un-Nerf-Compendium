...
public sealed class PK3 : G3PKM, ISanityChecksum
{
    ...
    // Gen 3 arbitrary-ability override (Jon/DraconicSOULS hackmons). Sanity byte 0x1E (low byte).
    // 0 = none (normal slot ability); value >= 2 forces that ability ID in-game (needs Emerald engine patch).
    // Batch Editor: .AbilityOverride=<id>  (e.g. 26 = Levitate). Outside the PK3 checksum.
