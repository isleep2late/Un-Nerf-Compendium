...
public sealed class PK1 : GBPKML, IPersonalType
{
    ...
    public override PK1 Clone()
    {
        ...
        clone.HeaderSpeciesInternal = HeaderSpeciesInternal; // PKHaX: carry the desynced sprite species
        return clone;
    }
    ...
    // PKHaX (PikaSav-style): Gen-1 stores the species twice -- once in the box/party "header" list
    // (this drives the on-screen sprite) and once in the data structure (SpeciesInternal). Vanilla
    // PKHeX keeps them equal; this lets them desync so a Pokemon can show any species' (or glitch)
    // sprite while remaining another species internally. 0 = synced to the data species. This value
    // lives in the save's list header (outside the 33-byte .pk1 body), so it persists in-save but
    // not across single .pk1 export/import.
    public byte HeaderSpeciesInternal { get; set; }
    /// <summary>True if the sprite (header) species has been desynced from the data species.</summary>
    public bool IsSpriteDesynced => HeaderSpeciesInternal != 0 && HeaderSpeciesInternal != SpeciesInternal;
    /// <summary>Effective raw Gen-1 index used to draw the sprite (header override if set, else data species).</summary>
    public byte SpriteSpeciesInternal => HeaderSpeciesInternal != 0 ? HeaderSpeciesInternal : SpeciesInternal;
