...
public sealed partial class PKMEditor : UserControl, IMainEditor
{
    ...
    // PKHaX: Gen-1 sprite/type hackmons editor (created in code, no Designer wiring).
    private readonly G1Editor G1_Editor = new();
    private readonly System.Windows.Forms.Label L_G1 = new() { Text = "Gen-1:", AutoSize = true, Anchor = System.Windows.Forms.AnchorStyles.Right, TextAlign = System.Drawing.ContentAlignment.MiddleRight };

    public PKMEditor()
    {
        InitializeComponent();

        // PKHaX: place the Gen-1 sprite/type editor on the free row 17 of the main table (mirrors Catch Rate at row 16).
        G1_Editor.AutoSize = true;
        G1_Editor.Dock = System.Windows.Forms.DockStyle.Fill;
        G1_Editor.Margin = new System.Windows.Forms.Padding(0, 1, 0, 1);
        TLP_Main.Controls.Add(L_G1, 0, 17);
        TLP_Main.Controls.Add(G1_Editor, 1, 17);
        G1_Editor.Visible = L_G1.Visible = false;
    ...
    private void SetAbilityList()
    {
        ...
        FieldsLoaded = false;
        if (Entity is PK3) // PKHaX: Gen-3 any-ability -- list every ability, not just the species' two slots
        {
            int prevVal = (CB_Ability.SelectedItem as ComboItem)?.Value ?? -1;
            var fullL = new System.Collections.Generic.List<ComboItem>(GameInfo.FilteredSources.Abilities);
            CB_Ability.DataSource = fullL;
            if (prevVal > 1) { int pidx = fullL.FindIndex(z => z.Value == prevVal); if (pidx >= 0) CB_Ability.SelectedIndex = pidx; }
        }
        else
        {
    ...
    private void UpdateEXPLevel(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                // PKHaX: EXP can only encode up to L100; peg EXP at the L100 minimum and write the byte the game uses.
                var expCapped = Experience.GetEXP(Experience.MaxLevel, gr);
                TB_EXP.Text = expCapped.ToString();
                Entity.Stat_Level = level;
                ExperienceBar.Update(expCapped, gr, Experience.MaxLevel);
            }
            else
            {
    ...
    private void UpdateRandomPID(object sender, EventArgs e)
    {
        ...
        if (sender == UC_Gender)
            ...
            Entity.SetPIDGender(Entity.Gender);
        else if (sender == CB_Ability && Entity is not PK3 /* PKHaX: skip PID-ability reset for Gen-3 any-ability */ && CB_Ability.SelectedIndex != Entity.PIDAbility && Entity.PIDAbility > -1)
    ...
    private void ValidateComboBox2(object? sender, EventArgs e)
    {
        ...
        if (sender == CB_Ability)
        {
            ...
            else if (Entity.Format <= 5 && Entity is not PK3 /* PKHaX: Gen-3 any-ability */ && CB_Ability.SelectedIndex < 2) // Format <= 5, not hidden
